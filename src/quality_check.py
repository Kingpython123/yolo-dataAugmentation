...
【参考缺陷规格 —— 评 shape_fidelity 时必须逐项对照】
  类型: {dtype}
  严重度: {severity}/5
  条数: ≈{count} 条
  走向: {orientation}
...
{{
  ...
  "shape_fidelity": 1-10,          // 形状/走向/条数 与参考的贴合度(必须严格按下面锚点打分)
...
- shape_fidelity: 逐项对照上面的参考规格(走向/条数/形态)严格打分, 不许含糊。
  这一项是本项目的核心目标(复刻参考形态), 之前因为提示语含糊导致分数一律挤在
  7~8 分、无法反映真实差异, 现按以下锚点执行:
  * 主走向与参考不一致(如参考是竖向或多向交叉, 生成的却是横向) -> 最高给 3。
  * 参考写明"多向交叉/交错/分叉", 而生成的是一组彼此近似平行、间距均匀的折痕
    (没有交叉、没有分叉) -> 最高给 3。这是最高频的失败模式, 务必抓出来。
  * 条数与参考相差一倍以上(如参考≈12条却只生成3~4条, 或反之) -> 最高给 4。
  * 参考写明形态不规则/弯曲/长短不一, 而生成的是规则等距的机械波纹, 或每条
    折痕深度完全一致 -> 最高给 4。
  * 只有走向、条数、疏密、规则程度、深浅变化都与参考基本吻合才可给 7 以上;
    需要几乎逐条对得上才给 9~10。
  * 不要因为"整体看起来像褶皱"就给高分 —— 像不像褶皱是 type_match 的职责,
    这一项只看与参考的一致性, 两者不要混为一谈。
